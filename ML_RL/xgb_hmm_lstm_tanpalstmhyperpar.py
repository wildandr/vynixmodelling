...
def combined_xgb_hmm_lstm_objective(trial):
    ...
    try:
        ...
        # LSTM hyperparameters - SELALU disertakan jika TensorFlow tersedia
        if TENSORFLOW_AVAILABLE:
            lstm_params = {
                'sequence_length': trial.suggest_int('sequence_length', 5, 70, step=5),
                'lstm_units': trial.suggest_int('lstm_units', 12, 128, step=8),
                'dropout_rate': trial.suggest_float('dropout_rate', 0.1, 0.5, step=0.005),
                'learning_rate': trial.suggest_float('lstm_learning_rate', 0.0001, 0.01, log=True),
                'batch_size': trial.suggest_categorical('batch_size', [8, 16, 32, 64, 128, 256, 512]),
                'epochs': trial.suggest_int('epochs', 10, 100, step=10)
            }
            
            # Ensemble weights - optimalkan bobot untuk setiap komponen
            ...
            # Pastikan lstm_weight minimal 0.1
            if lstm_weight < 0.1:
                lstm_weight = 0.1
        ...
        # TRAIN XGB-HMM-LSTM MODEL dengan parameter yang telah dioptimalkan
        if TENSORFLOW_AVAILABLE and lstm_params is not None:
            # Initialize XGB-HMM-LSTM model
            temp_xgb_hmm_lstm_model = XGBHMMLSTMModel(
                n_states=actual_n_states,
                max_iter=hmm_params['max_iter'],
                tol=hmm_params['tol'],
            ...
            temp_xgb_hmm_lstm_model.xgb_params = xgb_params
            
            # Set LSTM training parameters
            temp_xgb_hmm_lstm_model.batch_size = lstm_params.get('batch_size', 32)
            temp_xgb_hmm_lstm_model.epochs = lstm_params.get('epochs', 50)
            
            # Fit the XGB-HMM-LSTM model
            temp_xgb_hmm_lstm_model.fit(X_train_new, y_train_mapped_new)
            
            # Evaluate on validation set
            y_val_pred_new = temp_xgb_hmm_lstm_model.predict(X_val_new)
            val_accuracy_new = accuracy_score(y_val_mapped_new, y_val_pred_new)
            
            return val_accuracy_new
        else:
            # Fallback to XGB-HMM only
            temp_xgb_hmm_model = XGBHMMModel(
                n_states=actual_n_states,
                max_iter=hmm_params['max_iter'],
                tol=hmm_params['tol'],
...
# Update optimization to properly handle LSTM
logging.info("Combined HMM, XGBoost, and LSTM Parameters Tuning with Optuna")
logging.info("-" * 50)

if TENSORFLOW_AVAILABLE:
    logging.info("TensorFlow available - FORCING XGB-HMM-LSTM hybrid optimization")
    objective_function = combined_xgb_hmm_lstm_objective
    study_name = "xgb_hmm_lstm_barrier_parameters"
else:
...
combined_study.optimize(
    objective_function, 
    n_trials=10 if TENSORFLOW_AVAILABLE else 5,  # More trials for LSTM optimization
...
# Force LSTM usage if TensorFlow is available and set default parameters
if TENSORFLOW_AVAILABLE:
    if final_lstm_params is None:
        logging.info("Using default LSTM parameters since optimization didn't find LSTM params")
        final_lstm_params = {
            'sequence_length': 20,
            'lstm_units': 50,
            'dropout_rate': 0.2,
        ...
        final_ensemble_weights = [0.4, 0.3, 0.3]
...
# Update the LSTM model training section in XGBHMMLSTMModel
def _train_lstm_model(self, X, y):
    ...
    logging.info("Training LSTM model for sequence learning...")
    
    # Initialize LSTM model with current parameters
...
# Add _train_lstm_model method to XGBHMMLSTMModel
XGBHMMLSTMModel._train_lstm_model = _train_lstm_model

# Check if we should use LSTM hybrid or fallback to XGB-HMM
if TENSORFLOW_AVAILABLE and final_lstm_params is not None:
    logging.info("INITIALIZING XGB-HMM-LSTM with optimized LSTM parameters")
    ...
    # Set LSTM training parameters
    xgb_hmm_lstm_model.batch_size = final_lstm_params.get('batch_size', 32)
    xgb_hmm_lstm_model.epochs = final_lstm_params.get('epochs', 50)
    ...
    logging.info(f"  dropout_rate: {final_lstm_params['dropout_rate']}")
    logging.info(f"  batch_size: {final_lstm_params.get('batch_size', 32)}")
    logging.info(f"  epochs: {final_lstm_params.get('epochs', 50)}")
    ...
    logging.info("Starting XGB-HMM-LSTM training with EM algorithm and LSTM integration...")
    start_time = time.time()
    
    # Fit the model (akan memanggil _train_lstm_model secara otomatis)
    ...
    if hasattr(model_to_evaluate, 'lstm_model') and model_to_evaluate.lstm_model is not None:
        ...
        logging.info(f"✅ LSTM sequence length: {final_lstm_params['sequence_length']}")
        logging.info(f"✅ LSTM units: {final_lstm_params['lstm_units']}")
    else:
        logging.error(f"❌ LSTM model failed to integrate properly")
        logging.warning(f"Falling back to XGB-HMM only")

elif TENSORFLOW_AVAILABLE:
    logging.warning("TensorFlow available but LSTM params not optimized - using default LSTM parameters")
    
    # Force use default LSTM parameters
    default_lstm_params = {
        'sequence_length': 20,
        'lstm_units': 50,
        'dropout_rate': 0.2,
    ...
    default_ensemble_weights = [0.4, 0.3, 0.3]
    
    # Initialize XGB-HMM-LSTM model with default parameters
    xgb_hmm_lstm_model = XGBHMMLSTMModel(
        n_states=final_hmm_params['n_states'],
        max_iter=final_hmm_params['max_iter'],
        tol=final_hmm_params['tol'],
        sequence_length=default_lstm_params['sequence_length'],
        lstm_units=default_lstm_params['lstm_units'],
        dropout_rate=default_lstm_params['dropout_rate'],
        learning_rate=default_lstm_params['learning_rate'],
    ...
    logging.info("Training XGB-HMM-LSTM with default LSTM parameters...")
    start_time = time.time()
...
logging.info(f"  Convergence achieved: {hasattr(model_to_evaluate, 'convergence_info') and model_to_evaluate.convergence_info.get('converged', False)}")
logging.info(f"  Final log-likelihood: {model_to_evaluate.log_likelihood_history[-1] if hasattr(model_to_evaluate, 'log_likelihood_history') and model_to_evaluate.log_likelihood_history else 'N/A'}")
...
if 'feature_importance' in feature_summary and 'importance_scores' in feature_summary['feature_importance']:
    importance_scores = feature_summary['feature_importance']['importance_scores']
