# XGB-HMM-LSTM Implementation Classes
import pandas as pd
import numpy as np
import logging
import time
...
class LSTMFeatureExtractor:
    """
    LSTM Feature Extractor for temporal sequence modeling
    Extracts temporal features from time series data for hybrid model integration
    """
    
    def __init__(self, sequence_length=20, lstm_units=50, dropout_rate=0.2, 
                 bidirectional=True, random_state=42):
    ...
    def fit(self, X, y=None, validation_split=0.2, epochs=50, batch_size=32, verbose=0):
        """
        Fit LSTM feature extractor
        
        Parameters:
        X: Input features (2D array)
        y: Target labels (optional, for supervised feature learning)
        """
        if not TENSORFLOW_AVAILABLE:
        ...
        # Determine output dimension (for feature extraction)
        output_dim = min(64, self.feature_dim)  # Compressed feature representation
        ...
        self.model.compile(
            ...
            loss='mse',  # Unsupervised feature learning
            metrics=['mae']
        )
        
        # Prepare target for unsupervised learning (autoencoder-like)
        if y is None:
            # Use the last timestep as target for feature learning
            y_target = X_sequences[:, -1, :]  # Last timestep
            # Compress target to match output dimension
            if y_target.shape[1] > output_dim:
            ...
            if len(y_array) > len(X_sequences):
                y_target = y_array[-len(X_sequences):]
            else:
                y_target = y_array
            
            # Convert to categorical if classification
            if len(np.unique(y_target)) <= 10:  # Assume classification
                y_target = to_categorical(y_target, num_classes=output_dim)
                self.model.compile(
                    optimizer=Adam(learning_rate=0.001),
        ...
        self.is_fitted = True
        logging.info(f"LSTM feature extractor trained with {len(X_sequences)} sequences")
        
        return self
    
    def transform(self, X):
        """Extract LSTM features from input data"""
        if not TENSORFLOW_AVAILABLE or not self.is_fitted:
            logging.warning("LSTM not available or not fitted. Returning original features.")
            return X
        ...
        # Extract features
        lstm_features = self.model.predict(X_sequences, verbose=0)
        
        # If we have fewer sequences than original samples, pad with last features
        if len(lstm_features) < len(X_array):
            padding_needed = len(X_array) - len(lstm_features)
            last_features = lstm_features[-1:].repeat(padding_needed, axis=0)
            lstm_features = np.vstack([lstm_features, last_features])
    ...
    def get_feature_importance(self):
        """Get feature importance from LSTM model"""
        if not self.is_fitted or not TENSORFLOW_AVAILABLE:
            return None
        
        # Get weights from the first LSTM layer
        lstm_weights = self.model.layers[1].get_weights()[0]  # Input weights
        
        # Calculate importance as mean absolute weight per feature
...
class XGBHMMModel:
    """
    Hybrid XGBoost-HMM-LSTM Model for Sequential Classification
    Combines HMM temporal modeling with XGBoost emission probabilities and LSTM feature extraction
    """
    
    def __init__(self, n_states=3, max_iter=50, tol=1e-4, random_state=42, 
                 use_lstm=True, lstm_params=None):
        ...
        self.lstm_training_history = None
    ...
    def _train_xgb_emission_model(self, X, y, gamma):
        ...
        self.xgb_model = XGBClassifier(
            ...
            colsample_bylevel=0.8,  # Dropout untuk level-wise feature sampling
            colsample_bynode=0.8,   # Dropout untuk node-wise feature sampling
            reg_alpha=0.1,
            reg_lambda=1.0,
            random_state=self.random_state,
            n_jobs=-1,
    ...
    def fit(self, X, y):
        """Fit XGB-HMM-LSTM model using EM algorithm with LSTM feature extraction"""
        ...
        logging.info(f"Starting XGB-HMM-LSTM training with {self.n_states} states for {num_classes} classes...")
        
        # LSTM Feature Extraction (if enabled)
        X_features = X.copy()
        if self.use_lstm and TENSORFLOW_AVAILABLE:
            logging.info("Training LSTM feature extractor...")
            ...
            lstm_features = self.lstm_extractor.fit_transform(X, y, epochs=30, verbose=0)
            ...
            min_samples = min(len(X_original), len(lstm_features))
            X_combined = np.hstack([
                X_original[-min_samples:],
                lstm_features[-min_samples:]
            ])
            
            # Convert back to DataFrame with proper column names
            original_cols = X.columns.tolist() if hasattr(X, 'columns') else [f'orig_{i}' for i in range(X_original.shape[1])]
            lstm_cols = [f'lstm_{i}' for i in range(lstm_features.shape[1])]
            all_cols = original_cols + lstm_cols
            
            X_features = pd.DataFrame(X_combined, columns=all_cols)
            
            # Align y with the combined features
            if len(y) > min_samples:
                y = y.iloc[-min_samples:] if hasattr(y, 'iloc') else y[-min_samples:]
            
            logging.info(f"LSTM feature extraction completed. Combined features shape: {X_features.shape}")
            self.lstm_training_history = self.lstm_extractor.training_history
        else:
            if self.use_lstm and not TENSORFLOW_AVAILABLE:
                logging.warning("LSTM requested but TensorFlow not available. Using original features only.")
        
        # Initialize with GMM-HMM
        state_sequences, gamma = self._initialize_gmm_hmm(X_features, y)
        ...
        self.xgb_model = self._train_xgb_emission_model(X_features, y, gamma)
        ...
        for iteration in range(self.max_iter):
            ...
            alpha, beta, gamma = self._forward_backward_algorithm(X_features)
            ...
            # Update transition matrix
            self.transition_matrix = self._update_transition_matrix(X_features, gamma)
            
            # Update start probabilities
            self.start_prob = gamma[0] / np.sum(gamma[0])
            
            # Compute log likelihood
            current_log_likelihood = self._compute_log_likelihood(X_features)
        ...
        logging.info("XGB-HMM-LSTM training completed")
        return self
    ...
    def predict(self, X):
        ...
        # Prepare features for prediction (including LSTM if enabled)
        X_prepared = self._prepare_features_for_prediction(X)
        
        # Get emission probabilities
        emission_probs = self._compute_emission_probabilities(X_prepared)
    ...
    def _prepare_features_for_prediction(self, X):
        """Prepare features for prediction, including LSTM feature extraction if enabled"""
        if self.use_lstm and self.lstm_extractor is not None:
            # Extract LSTM features
            lstm_features = self.lstm_extractor.transform(X)
    ...
    def predict_proba(self, X):
        ...
        alpha, beta, gamma = self._forward_backward_algorithm(X_prepared)
        return gamma
...
log_filename = f'/root/vynixmodelling/ML_RL/logs/main/lstm_{current_datetime}.log'
...
def combined_xgb_hmm_objective(trial):
    """Combined objective function for HMM, XGBoost, LSTM, and Barrier parameters"""
    try:
        # HMM hyperparameters
        hmm_params = {
        ...
        if TENSORFLOW_AVAILABLE:
            ...
            if use_lstm:
                lstm_params = {
                    'sequence_length': trial.suggest_int('lstm_sequence_length', 5, 70, step=5),
                    'lstm_units': trial.suggest_int('lstm_units', 16, 512, step=16),
                    'dropout_rate': trial.suggest_float('lstm_dropout_rate', 0.01, 0.5, step=0.01),
                    'bidirectional': trial.suggest_categorical('lstm_bidirectional', [True, False]),
                    'epochs': trial.suggest_int('lstm_epochs', 10, 1000, step=10),
                    'batch_size': trial.suggest_int('lstm_batch_size', 16, 512, step=8),
                    'validation_split': trial.suggest_float('lstm_validation_split', 0.1, 0.3, step=0.05)
                }
...
combined_study = create_study_with_storage(
    study_name="combined_main_xgb_hmm_lstm_barrier_parameters",
    direction='maximize'
)
...
if TENSORFLOW_AVAILABLE:
    best_lstm_params_combined = {
        'sequence_length': best_combined_trial.params.get('lstm_sequence_length', 20),
        'lstm_units': best_combined_trial.params.get('lstm_units', 50),
        'dropout_rate': best_combined_trial.params.get('lstm_dropout_rate', 0.2),
        'bidirectional': best_combined_trial.params.get('lstm_bidirectional', True),
        'epochs': best_combined_trial.params.get('lstm_epochs', 50),
        'batch_size': best_combined_trial.params.get('lstm_batch_size', 32),
        'validation_split': best_combined_trial.params.get('lstm_validation_split', 0.2)
    }
...
if TENSORFLOW_AVAILABLE and best_lstm_params_combined:
...
optimization_summary = {
    ...
    'best_lstm_params': best_lstm_params_combined if TENSORFLOW_AVAILABLE else {}
}
...
final_lstm_params = best_lstm_params_combined if TENSORFLOW_AVAILABLE else {}
...
logging.info("\n=== Training XGB-HMM Hybrid Model with Selected Features ===")
logging.info("Initializing XGB-HMM model with optimal parameters and selected features...")
...
xgb_hmm_model = XGBHMMModel(
    ...
    random_state=42,
    use_lstm=TENSORFLOW_AVAILABLE and bool(final_lstm_params),
    lstm_params=final_lstm_params if TENSORFLOW_AVAILABLE else None
)
...
logging.info(f"  use_lstm: {TENSORFLOW_AVAILABLE and bool(final_lstm_params)}")
if TENSORFLOW_AVAILABLE and final_lstm_params:
    logging.info(f"  LSTM parameters: {final_lstm_params}")
...
train_evaluation = xgb_hmm_model.evaluate_model(X_train_selected, y_train_mapped_final)
logging.info(f"\nTraining Set XGB-HMM Evaluation:")
for metric, value in train_evaluation.items():
    logging.info(f"  {metric}: {value:.4f}")

# Evaluate on validation set
val_evaluation = xgb_hmm_model.evaluate_model(X_val_selected, y_val_mapped_final)
logging.info(f"\nValidation Set XGB-HMM Evaluation:")
for metric, value in val_evaluation.items():
    logging.info(f"  {metric}: {value:.4f}")

# Evaluate on test set
test_evaluation = xgb_hmm_model.evaluate_model(X_test_selected, y_test_mapped_final)
...
logging.info("\n=== State Transition Analysis ===")
transition_analysis = xgb_hmm_model.get_state_transition_analysis(X_test_selected)
...
y_train_pred_final = xgb_hmm_model.predict(X_train_selected)
y_train_proba_final = xgb_hmm_model.predict_proba(X_train_selected)
...
y_val_pred_final = xgb_hmm_model.predict(X_val_selected)
y_val_proba_final = xgb_hmm_model.predict_proba(X_val_selected)
...
# Test set evaluation
y_test_pred_final = xgb_hmm_model.predict(X_test_selected)
y_test_proba_final = xgb_hmm_model.predict_proba(X_test_selected)
...
logging.info("\n" + "="*80)
logging.info("XGB-HMM HYBRID MODEL WITH FEATURE SELECTION COMPLETED SUCCESSFULLY!")
logging.info("="*80)